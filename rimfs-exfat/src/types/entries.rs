...
use crate::{
    ...
    {attr::ExFatFileAttributesExt, constant::*, upcase::UpcaseHandle},
};
...
impl ExFatEntries {
    ...
    pub fn dir(
        ...
        let mut stream = ExFatStreamEntry::new(cluster, 0, name_length, name_hash);
        if cluster != 0 {
...
impl ExFatPrimaryEntry {
    pub fn new(attr: &FileAttributes, secondary_count: u8) -> Self {
        ...
        let (c_time, c_fine, c_utc) = created.to_exfat_datetime();
        let (m_time, m_fine, m_utc) = modified.to_exfat_datetime();
        let (a_time, _, a_utc) = accessed.to_exfat_datetime();
