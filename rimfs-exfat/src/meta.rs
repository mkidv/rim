...
use crate::{
    core::{FsError, FsResult, bitmap::BitmapFsMeta, fat::FatFsMeta},
    {constant::*, types::*, upcase::UpcaseFlavor},
};
...
impl ExFatMeta {
    ...
    pub fn from_io<IO: RimIO + ?Sized>(io: &mut IO) -> FsResult<Self> {
        ...
        if &vbr.fs_name != EXFAT_FS_NAME {
            return Err(FsError::Invalid("Invalid ExFAT VBR: fs_name mismatch"));
        }

        if vbr.signature != [0x55, 0xAA] {
            return Err(FsError::Invalid(
                "Invalid ExFAT VBR: boot signature missing",
            ));
        }
        ...
        if !(9..=12).contains(&vbr.bytes_per_sector_shift) {
            return Err(FsError::Invalid(
                "Invalid ExFAT VBR: invalid sector size shift",
            ));
        }

        if vbr.sectors_per_cluster_shift > 25 {
            return Err(FsError::Invalid(
                "Invalid ExFAT VBR: invalid cluster size shift",
            ));
        }

        let bytes_per_sector = 1u32 << vbr.bytes_per_sector_shift;
        let sectors_per_cluster = 1u32 << vbr.sectors_per_cluster_shift;
        // Avoid huge allocations on invalid data
        if !(512..=4096).contains(&bytes_per_sector) {
            return Err(FsError::Invalid("Invalid ExFAT VBR: invalid sector size"));
        }
...
impl FsMeta<u32> for ExFatMeta {
    ...
    fn first_data_unit(&self) -> u32 {
        // System structures can be anywhere in exFAT.
        // A truly robust 'first data unit' would depend on the actual bitmap content.
        // For metadata purposes, we use a conservative guess based on known system files.
        let end_bm = self.bitmap_cluster + self.bitmap_clusters();
        let end_uc = self.upcase_cluster + self.upcase_clusters();
        let end_rt = self.root_cluster + self.root_clusters();
        end_bm.max(end_uc).max(end_rt).max(Self::FIRST_CLUSTER)
    }
...
impl FatFsMeta for ExFatMeta {
    ...
    fn entry_mask(&self) -> u32 {
        EXFAT_MASK // ExFAT uses all 32 bits
    }
