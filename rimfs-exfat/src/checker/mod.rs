...
fn check_fat_chains_deep<IO: RimIO + ?Sized>(io: &mut IO, meta: &ExFatMeta) -> FsCheckerResult {
    ...
    for start in first_cluster..last_cluster {
        ...
        while (EXFAT_FIRST_CLUSTER..EXFAT_EOC).contains(&current) {
            ...
            let next = driver.read_entry(io, current)?;
            chain_len += 1;
...
fn verify_upcase_checksum_over_file<IO: RimIO + ?Sized>(
    ...
    while remain > 0 {
        ...
        // Follow FAT chain
        let next = driver.read_entry(io, cur)?;
        if next == EXFAT_EOC {
...
fn bitmap_has_cluster_meta<IO: RimIO + ?Sized>(
    ...
    let cluster_offset_in_bitmap = (byte_index / cluster_size) as u32;
    let byte_in_cluster = (byte_index % cluster_size) as usize;

    let mut target_cluster = bfc;
    if cluster_offset_in_bitmap > 0 {
        let mut driver = FatDriver::new(meta);
        let mut followed_chain = true;
        for _ in 0..cluster_offset_in_bitmap {
            let next = driver.read_entry(io, target_cluster)?;
            if (EXFAT_FIRST_CLUSTER..EXFAT_EOC).contains(&next) {
                target_cluster = next;
            } else {
                followed_chain = false;
                break;
            }
        }
        if !followed_chain {
            // Contiguous bitmap allocation fallback
            target_cluster = bfc + cluster_offset_in_bitmap;
        }
    }

    if target_cluster < EXFAT_FIRST_CLUSTER || target_cluster > meta.last_data_unit() {
        return Ok(false);
    }

    let mut clus = vec![0u8; cluster_size as usize];
    io.read_at(meta.unit_offset(target_cluster), &mut clus)?;
    let byte = clus[byte_in_cluster];
    let bit = (idx % 8) as u8;
    Ok((byte & (1 << bit)) != 0)
}
...
mod tests {
    ...
    #[test]
    fn test_multi_cluster_bitmap_tracking() {
        let meta = ExFatMeta::new(128 * 1024 * 1024, None).unwrap();
        let cluster_size = (meta.bytes_per_sector as usize) * (meta.sectors_per_cluster as usize);
        let mut disk = vec![0u8; 128 * 1024 * 1024];
        let mut io = MemRimIO::new(&mut disk);

        let bfc = meta.bitmap_cluster;
        let blen = 2 * cluster_size as u64;
        ...
        second_clus_data[0] = 0x01; // first bit set
        io.write_at(second_bmp_offset, &second_clus_data).unwrap();

        let allocated =
            bitmap_has_cluster_meta(&mut io, &meta, bfc, blen, cluster_in_2nd_bmp_clus).unwrap();
        assert!(
            allocated,
            "Cluster in 2nd bitmap cluster should be reported as allocated"
        );

        let unallocated =
            bitmap_has_cluster_meta(&mut io, &meta, bfc, blen, cluster_in_2nd_bmp_clus + 1)
                .unwrap();
        assert!(
            !unallocated,
