...
fn test_exfat_boot_sector_wire_format_offsets() {
    ...
    assert_eq!(&bytes[0..3], &[0xEB, 0x76, 0x90], "Jump instruction at 0x00");
    assert_eq!(&bytes[3..11], b"EXFAT   ", "FS name at 0x03");
    assert_eq!(&bytes[100..104], &meta.volume_id.to_le_bytes(), "Volume serial at 0x64");
    assert_eq!(&bytes[104..106], &[0x00, 0x01], "FS revision 1.00 at 0x68");
    assert_eq!(bytes[108], meta.bytes_per_sector.trailing_zeros() as u8, "Sector shift at 0x6C");
    assert_eq!(bytes[109], meta.sectors_per_cluster.trailing_zeros() as u8, "Cluster shift at 0x6D");
