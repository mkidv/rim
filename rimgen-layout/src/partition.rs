// rimgen/src/layout/partition.rs

use crate::filesytem::Filesystem;
use crate::size::Size;
...
impl Partition {
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        if matches!(self.fs, Filesystem::Raw | Filesystem::None) && !self.mountpoint.is_empty() {
            anyhow::bail!(
        ...
        if self.name.len() > 36 {
            println!(
                "[rimgen] Warning: partition name '{}' truncated to 36 characters",
                self.name
            );
        }
        ...
        if !self.name.is_ascii() {
            println!(
                "[rimgen] Warning: ASCII character of partition name '{}' will be ignored",
                self.name
            );
        }
        ...
        if self.kind.is_some() && effective_kind != expected_kind {
            println!(
                "[rimgen] Warning: Partition '{}' uses fs '{}' but type '{:?}' (expected {:?})",
