...
mod tests {
    ...
    use rimfs_core::testing::{
        ExpectedFile, ExpectedLink, assert_dir_entries, assert_exists, assert_files,
        assert_has_error, assert_no_findings, assert_symlinks, file, file_with_attr,
    };
    use rimio::{MemRimIO, RimRead, RimWrite, SliceRimIO};

    #[test]
    fn test_iso_empty_format_roundtrip() {
    ...
    fn test_iso_el_torito_efi_boot() {
        ...
        let mut tree =
            FsNode::new_container(alloc::vec![file("readme.txt", b"Bootable ISO Image",)]);
