...
fn bench_iso_format(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_format");
...
fn bench_iso_large_write(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_write_large");
    let meta = IsoMeta::new(SIZE_BYTES, Some("BENCH_ISO")).unwrap();
...
fn bench_iso_large_read(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_read_large");
    let meta = IsoMeta::new(SIZE_BYTES, Some("BENCH_ISO")).unwrap();
    let mut disk_buf = vec![0u8; SIZE_BYTES as usize];
    {
        let mut io = MemRimIO::new(&mut disk_buf);
        ...
        let mut node = FsNode::Container {
            attr: FileAttributes::new_dir(),
            children: vec![FsNode::new_file("large.bin", content)],
        };
        injector.inject_tree(&mut node).unwrap();
        injector.flush().unwrap();
    }
...
fn bench_iso_small_files(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_small_files");
    let meta = IsoMeta::new(SIZE_BYTES, Some("BENCH_ISO")).unwrap();
    let mut disk_buf = vec![0u8; SIZE_BYTES as usize];
    {
        let mut io = MemRimIO::new(&mut disk_buf);
...
fn bench_iso_check(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_check");
    let meta = IsoMeta::new(SIZE_BYTES, Some("BENCH_ISO")).unwrap();
    let mut disk_buf = vec![0u8; SIZE_BYTES as usize];
    {
        let mut io = MemRimIO::new(&mut disk_buf);
...
fn bench_iso_resolve_tree(c: &mut Criterion) {
    let mut group = c.benchmark_group("iso_resolve_tree");
    let meta = IsoMeta::new(SIZE_BYTES, Some("BENCH_ISO")).unwrap();
    let mut disk_buf = vec![0u8; SIZE_BYTES as usize];
    {
        let mut io = MemRimIO::new(&mut disk_buf);
...
criterion_group!(
    benches,
    bench_iso_format,
    bench_iso_large_write,
    bench_iso_large_read,
    bench_iso_small_files,
    bench_iso_check,
    bench_iso_resolve_tree,
);
