...
impl FsAllocatorError {
    pub fn msg(&self) -> &'static str {
        match self {
            FsAllocatorError::IO(_) => "IO error",
            FsAllocatorError::InvalidSize => "Invalid size",
...
impl FsFormatterError {
    pub fn msg(&self) -> &'static str {
        match self {
            ...
            FsFormatterError::Allocator(_) => "Allocator error",
            FsFormatterError::Feature(_) => "Feature error",
    ...
    pub fn source(&self) -> Option<FsError> {
        match self {
            ...
            FsFormatterError::Allocator(e) => Some(FsError::Allocator(*e)),
            FsFormatterError::Feature(e) => Some(FsError::Feature(*e)),
            _ => None,
        }
    }
}
...
impl From<FsInjectorError> for FsFormatterError {
    fn from(e: FsInjectorError) -> Self {
        match e {
            FsInjectorError::IO(io) => FsFormatterError::IO(io),
            FsInjectorError::Allocator(alloc) => FsFormatterError::Allocator(alloc),
            FsInjectorError::Invalid(msg) => FsFormatterError::Invalid(msg),
            FsInjectorError::Other(msg) => FsFormatterError::Other(msg),
            _ => FsFormatterError::Other("Injector error during formatting"),
        }
    }
}
...
impl FsInjectorError {
    ...
    pub fn source(&self) -> Option<FsError> {
        match self {
            ...
            FsInjectorError::Feature(e) => Some(FsError::Feature(*e)),
            _ => None,
        }
    }
}
...
impl FsCheckerError {
    pub fn msg(&self) -> &'static str {
        match self {
            ...
            FsCheckerError::Feature(_) => "Feature error",
    ...
    pub fn source(&self) -> Option<FsError> {
        match self {
            ...
            FsCheckerError::Feature(e) => Some(FsError::Feature(*e)),
            _ => None,
        }
    }
}
...
impl FsFeatureError {
    pub fn msg(&self) -> &'static str {
        match self {
            FsFeatureError::IO(_) => "IO error",
            FsFeatureError::Allocator(_) => "Allocator error",
            FsFeatureError::NotPrepared => "Feature not prepared",
            FsFeatureError::InvalidConfiguration(msg) => msg,
            FsFeatureError::Other(msg) => msg,
        }
    }

    pub fn source(&self) -> Option<FsError> {
        match self {
            FsFeatureError::IO(e) => Some(FsError::IO(*e)),
            FsFeatureError::Allocator(e) => Some(FsError::Allocator(*e)),
            _ => None,
        }
    }
}

impl fmt::Display for FsFeatureError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.msg())?;
        let mut current = self.source();
        while let Some(src) = current {
...
pub type FsFeatureResult<T = ()> = Result<T, FsFeatureError>;
...
crate::fs_error_wiring! {
    ...
    sub => {
        RimIOError     => [FsAllocatorError::IO, FsResolverError::IO, FsFormatterError::IO, FsInjectorError::IO, FsCheckerError::IO, FsFeatureError::IO, FsCursorError::IO ],
        FsAllocatorError => [ FsInjectorError::Allocator, FsFormatterError::Allocator, FsFeatureError::Allocator ],
        FsFeatureError    => [ FsFormatterError::Feature, FsInjectorError::Feature, FsCheckerError::Feature ],
    ...
    impl std::error::Error for FsFormatterError {
        fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
            match self {
                ...
                FsFormatterError::Allocator(e) => Some(e),
                FsFormatterError::Feature(e) => Some(e),
                _ => None,
            }
        }
    }
    ...
    impl std::error::Error for FsInjectorError {
        fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
            match self {
                ...
                FsInjectorError::Feature(e) => Some(e),
                _ => None,
            }
        }
    }
    ...
    impl std::error::Error for FsCheckerError {
        fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
            match self {
                ...
                FsCheckerError::Feature(e) => Some(e),
                _ => None,
            }
        }
    }

    impl std::error::Error for FsFeatureError {
        fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
            match self {
                FsFeatureError::IO(e) => Some(e),
                FsFeatureError::Allocator(e) => Some(e),
                _ => None,
            }
        }
    }
