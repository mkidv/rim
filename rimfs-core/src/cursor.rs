    ...
    M: FatFsMeta,
{
    ...
    fn in_bounds(&self, c: u32) -> bool {
        if self.allow_system && c == self.meta.root_unit() {
            return true;
        }
...
pub struct LinearCursor<'a, M: FatFsMeta> {
    meta: &'a M,
    next: u32,
    end_excl: u32,
    allow_system: bool,
...
impl<'a, M: FatFsMeta> Iterator for LinearCursor<'a, M> {
    type Item = FsCursorResult<u32>;
