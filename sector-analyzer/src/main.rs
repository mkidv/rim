...
fn main() {
    ...
    println!("\nexFAT partition starts at offset: 0x{:X}", partition_offset);
    ...
    for sector in 0..12 {
        ...
        io.read_at(sector_offset, &mut buffer).expect("Failed to read sector");
        ...
        if sector == 0 {
            println!("Jump Boot: {:02X} {:02X} {:02X}", buffer[0], buffer[1], buffer[2]);
            ...
            if buffer.len() >= 120 {
                ...
                println!("Bytes per sector shift: {} -> {} bytes", bytes_per_sector_shift, bytes_per_sector);
                println!("Sectors per cluster shift: {} -> {} sectors", sectors_per_cluster_shift, sectors_per_cluster);
                ...
                let fat_offset = u32::from_le_bytes([buffer[80], buffer[81], buffer[82], buffer[83]]);
                let fat_length = u32::from_le_bytes([buffer[84], buffer[85], buffer[86], buffer[87]]);
                let cluster_heap_offset = u32::from_le_bytes([buffer[88], buffer[89], buffer[90], buffer[91]]);
                let cluster_count = u32::from_le_bytes([buffer[92], buffer[93], buffer[94], buffer[95]]);
                let root_dir_cluster = u32::from_le_bytes([buffer[96], buffer[97], buffer[98], buffer[99]]);
                let volume_serial = u32::from_le_bytes([buffer[100], buffer[101], buffer[102], buffer[103]]);
...
fn analyze_root_cluster(io: &mut StdBlockIO<File>, partition_offset: u64) {
    println!("\n=== ROOT DIRECTORY CLUSTER ANALYSIS ===");
    
    // Get actual values from boot sector
    let mut boot_buffer = [0u8; 512];
    io.read_at(partition_offset, &mut boot_buffer).expect("Failed to read boot sector");
    
    let cluster_heap_offset = u32::from_le_bytes([boot_buffer[88], boot_buffer[89], boot_buffer[90], boot_buffer[91]]);
    let root_cluster = u32::from_le_bytes([boot_buffer[96], boot_buffer[97], boot_buffer[98], boot_buffer[99]]);
    ...
    println!("Root cluster {} starts at sector: {}", root_cluster, root_cluster_sector);
    ...
    let mut buffer = [0u8; 512];
    io.read_at(sector_offset, &mut buffer).expect("Failed to read root cluster sector");
...
fn analyze_disk_structure(io: &mut StdBlockIO<File>) -> u64 {
    println!("\n=== DISK STRUCTURE ANALYSIS ===");
    
    // Parse MBR using rimpart
    match mbr::parse_mbr(io) {
        Ok(mbr_data) => {
            println!("MBR signature: 0x{:04X} ✓", u16::from_le_bytes(mbr_data.signature));
            println!("GPT Protective MBR detected ✓");
            
            // Parse GPT using rimpart
            match gpt::parse_gpt(io) {
                Ok((header, partitions)) => {
                    println!("GPT header signature: {} ✓", std::str::from_utf8(&header.signature).unwrap());
                    ...
                    if let Some(first_partition) = partitions.first() {
                        ...
                        println!("First partition: LBA {} - {} (offset 0x{:X})", 
