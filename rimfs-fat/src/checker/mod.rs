...
pub struct FatChecker<'a, IO: RimIO + ?Sized> {
    io: &'a mut IO,
    meta: &'a FatMeta,
}
impl<'a, IO: RimIO + ?Sized> FatChecker<'a, IO> {
...
impl<'a, IO: RimIO + ?Sized> FsChecker for FatChecker<'a, IO> {
    ...
    fn check_boot(&mut self, opt: &Self::Options, rep: &mut VerifyReport) -> FsCheckerResult<()> {
        ...
        if opt.check_fsinfo_consistency || self.meta.use_integrity {
    ...
    fn check_root(&mut self, _opt: &Self::Options, rep: &mut VerifyReport) -> FsCheckerResult<()> {
        ...
        if self.meta.root_entry_count == 0
            && FatDriver::new(self.meta).read_entry(self.io, self.meta.root_unit())? == 0
        {
    ...
    fn check_cross_reference(
        ...
        if opt.walk_reachability {
            let mut walker = walker::FatWalker::new(self.io, self.meta);
