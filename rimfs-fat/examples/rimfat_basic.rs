...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let meta = FatMeta::new_rimfat(SIZE_BYTES, Some("RIMFAT"))?;
    ...
    let mut tree = FsNode::new_container(vec![FsNode::new_file(
        "integrity.txt",
        b"RimFAT keeps extra integrity metadata.\n".to_vec(),
    )]);
    ...
    assert_eq!(
        resolver.read_file("integrity.txt")?,
        b"RimFAT keeps extra integrity metadata.\n"
    );
    let resolve = (t.elapsed(), resolve_io.snapshot());

    println!("RimFAT example completed in {:?}", total.elapsed());
