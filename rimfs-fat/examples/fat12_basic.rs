...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let meta = FatMeta::new_fat12(SIZE_BYTES, Some("RIMFAT12"))?;
    ...
    let mut tree = FsNode::new_container(vec![
        FsNode::new_file("hello.txt", b"hello from FAT12\n".to_vec()),
        FsNode::new_file("notes.txt", b"small volumes still count\n".to_vec()),
    ]);
    ...
    assert_eq!(resolver.read_file("hello.txt")?, b"hello from FAT12\n");
    let resolve = (t.elapsed(), resolve_io.snapshot());

    println!("FAT12 example completed in {:?}", total.elapsed());
