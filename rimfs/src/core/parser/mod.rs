...
pub use crate::core::error::{FsParserError, FsParserResult};
...
/// - The real filesystem (see [`parser_std`])
...
pub trait FsParser {
    ...
    fn build_node(&mut self, path: &str, recurse: bool) -> FsParserResult<FsNode> {
        if is_wildcard(path) {
    ...
    fn parse_tree(&mut self, path: &str) -> FsParserResult<FsNode> {
    ...
    fn parse_path(&mut self, path: &str) -> FsParserResult<FsNode> {
