...
pub struct StdFsParser {}

#[cfg(feature = "std")]
impl StdFsParser {
    /// Creates a new [`StdFsParser`].
    pub fn new() -> Self {
        Self {}
    }
}
...
impl FsParser for StdFsParser {
    ...
    fn read_dir(&mut self, path: &str) -> FsParserResult<Vec<String>> {
        ...
        for entry in fs::read_dir(path)? {
            ...
            if let Some(name) = entry.file_name().to_str() {
                ...
                return Err(FsParserError::Unsupported);
            }
        }
        res.sort_unstable();
        Ok(res)
    ...
    fn read_attributes(&mut self, path: &str) -> FsParserResult<FileAttributes> {
        ...
        let name = path
            .file_name()
            .and_then(|n| n.to_str())
            .ok_or(FsParserError::Unsupported)?;
...
#[cfg(feature = "std")]
impl From<std::io::Error> for FsParserError {
    fn from(e: std::io::Error) -> Self {
        let leaked_str: &'static str = Box::leak(e.to_string().into_boxed_str());
        FsParserError::IO(BlockIOError::Error(leaked_str))
    }
}
