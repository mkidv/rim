...
fn main() {
    ...
    println!("=== NTFS Test - {SIZE_MB} MB image ===");

    let mut buf = vec![0u8; SIZE_BYTES as usize];
    let mut mem = MemRimIO::new(&mut buf);
    let meta = NtfsMeta::new(SIZE_BYTES, Some("BENCHFS")).unwrap();

    // Alignement pertinent : cluster Ntfs, sinon 4096.
    let align = meta.bytes_per_cluster as u64; // ou 4096
    let mut io_for_format = IOCounter::with_align(&mut mem, align);

    // FORMAT
    let t0 = Instant::now();
    let mut formatter = NtfsFormatter::new(&mut io_for_format, &meta);
    ...
    let t3 = Instant::now();
    let mut checker = NtfsChecker::new(&mut io_for_check, &meta);
